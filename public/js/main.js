    ...
    async manejarSubmitVenta(e) {
        ...
        try {
            ...
            if (resultado.exito) {
                ...
                if (this.ventaEnEdicion && this._equipoInventarioIdAnterior) {
                    // EDICIÓN: si el equipo cambió, revertir el anterior y marcar el nuevo
                    if (this._equipoInventarioId !== this._equipoInventarioIdAnterior) {
                        // Revertir el equipo anterior a disponible
                        ...
                        // Marcar el nuevo equipo como vendido
                        if (this._equipoInventarioId) {
                            inventarioService.marcarVendido(this._equipoInventarioId, ventaId)
                                .catch(err => console.warn('⚠️ No se pudo marcar equipo nuevo como vendido:', err));
                        }
                    }
                ...
                // ── INVENTARIO: Auto-ingreso de Trade-In (Equipo Recibido) ──
                if (!this.ventaEnEdicion && datosVenta.equipoRecibido && datosVenta.equipoRecibido.imei) {
                    // Verificamos por IMEI si ya existe, por si fue doble submit
                    const eqExiste = inventarioService.buscarPorImei(datosVenta.equipoRecibido.imei);
                    if (!eqExiste) {
                        const eqRecibido = new EquipoInventario({
                            tipoItem: 'equipo',
                            modelo: datosVenta.equipoRecibido.modelo,
                            gb: datosVenta.equipoRecibido.capacidad,
                            color: datosVenta.equipoRecibido.color,
                            bateria: parseInt(datosVenta.equipoRecibido.bateria) || 0,
                            imei: datosVenta.equipoRecibido.imei,
                            detalles: datosVenta.equipoRecibido.comentarios || '',
                            origen: `Trade-in (Venta: ${datosVenta.cliente?.nombre || 'Anónimo'})`,
                            estado: 'disponible'
                        });
                        inventarioService.ingresarEquipo(eqRecibido)
                            .catch(err => console.warn('⚠️ Error al ingresar Trade-in al inventario:', err));
                    }
                }
                // ────────────────────────────────────────────────────────────
